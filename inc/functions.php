...
// Section Heading
function anipat_section_heading( $title = '', $subtitle = '' ) {
	if( $title || $subtitle ) :
	?>
		<div class="container">
			<div class="row">
...
// Enqueue scripts
add_action( 'wp_enqueue_scripts', 'anipat_companion_frontend_scripts', 99 );
function anipat_companion_frontend_scripts() {

	wp_enqueue_script( 'anipat-companion-script', plugins_url( '../js/loadmore-ajax.js', __FILE__ ), array( 'jquery' ), '1.0', true );
	wp_enqueue_script( 'anipat-common-js', plugins_url( '../js/common.js', __FILE__ ), array( 'jquery' ), '1.0', true );

}
// 
add_action( 'wp_ajax_anipat_portfolio_ajax', 'anipat_portfolio_ajax' );

add_action( 'wp_ajax_nopriv_anipat_portfolio_ajax', 'anipat_portfolio_ajax' );
function anipat_portfolio_ajax( ){
	...
	function anipat_update_the_followed_post_page_status( $title = 'Hello world!', $type = 'post', $status = 'draft', $message = false ){
...
/*=========================================================
    Cases Section
========================================================*/
function anipat_case_section( $post_order ){ 
	...
	if( $cases->have_posts() ) {
		while ( $cases->have_posts() ) {
			...
			$case_img      = get_the_post_thumbnail( get_the_ID(), 'anipat_case_study_thumb_362x240', '', array( 'alt' => get_the_title() ) );
			?>
...
// Related anipat for Single Page
function anipat_related_items( $current_post_id = null, $post_item = 3, $post_order = 'DESC', $have_related_listing_title = 'Related Listings' ){
			...
			<div class="row">
				...
				if( $related_listings->have_posts() ) {
					while ( $related_listings->have_posts() ) {
						...
						$recipe_img = get_the_post_thumbnail( get_the_ID(), 'anipat_listing_thumb_362x250', '', array( 'alt' => get_the_title() ) );
						$listing_address = ! empty( anipat_meta( 'listing_address') ) ? anipat_meta( 'listing_address') : 'N/A';
						$phone_number = ! empty( anipat_meta( 'phone_number') ) ? anipat_meta( 'phone_number') : 'N/A';
						$listing_email = ! empty( anipat_meta( 'listing_email') ) ? anipat_meta( 'listing_email') : 'N/A';
						?>
...
function anipat_get_tabbed_contents( $sec_title = 'Explore Europe', $selected_countries ) {
	$i = 0;
	?>
						...
						<div class="row">
							...
							if( $tabbed_listings->have_posts() ) {
								while ( $tabbed_listings->have_posts() ) {
									...
									$listing_img = get_the_post_thumbnail( get_the_ID(), 'anipat_listing_thumb_362x250', '', array( 'alt' => get_the_title() ) );
									$listing_address = ! empty( anipat_meta( 'listing_address') ) ? anipat_meta( 'listing_address') : 'N/A';
									$phone_number = ! empty( anipat_meta( 'phone_number') ) ? anipat_meta( 'phone_number') : 'N/A';
									$listing_email = ! empty( anipat_meta( 'listing_email') ) ? anipat_meta( 'listing_email') : 'N/A';
									?>
...
function set_custom_edit_listing_columns($columns) {
    ...
    $columns['listing_price'] = __( 'Listing Price', 'anipat_companion' );
    $columns['listing_area'] = __( 'Listing Area', 'anipat_companion' );
...
function custom_listing_column( $column, $post_id ) {
    ...
    switch ( $column ) {
        ...
        case 'listing_price' :
            echo get_post_meta( $post_id , '_anipat_listing_price' , true ); 
            break;

        case 'listing_area' :
            echo get_post_meta( $post_id , '_anipat_listing_area' , true ); 
            break;
...
if ( ! function_exists( 'search_prop_form_datas' ) ) {
	function search_prop_form_datas() {
		...
		if ( $search_text == '' || $search_category == '' || $search_location == '' || $search_area_from == '' || $search_area_to == '' || $price_min == '' || $price_max == '' ) {
			...
			$response['message']  = __( 'Sorry! Empty field is not allowed.', 'anipat-companion' );
		} else {
			$item = 0;
			...
			$properties = new WP_Query( array(
				...
				'meta_query' => array(
					'relation' => 'AND',
					array(
						'key'     => '_anipat_listing_price',
						'value'   => array($price_min, $price_max),
						'type'    => 'numeric',
						'compare' => 'BETWEEN',
					),
					array(
						'key'     => '_anipat_listing_area',
			...
			if( $properties->have_posts() ) {
				while ( $properties->have_posts() ) {
					$properties->the_post();		
					$property_img = get_the_post_thumbnail( get_the_ID(), 'anipat_listing_thumb_362x250', '', array( 'alt' => get_the_title() ) );
